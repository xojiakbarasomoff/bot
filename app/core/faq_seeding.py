...
async def resolve_faq_tenant_id(
    ...
    Resolved from the Instagram channel rather than taken as a raw UUID: a
    mistyped UUID would seed a tenant nobody serves, and the rows would then
    be invisible to every query the webhook makes -- a failure that looks
    exactly like the FAQs never having been loaded at all.
    """
    query = select(Channel).where(Channel.type == CHANNEL_TYPE)
    ...
    if not channels:
        ...
        raise FaqSeedingError(f"No {CHANNEL_TYPE} channel found{which}.")
    if len(channels) > 1:
        ids = ", ".join(sorted(channel.external_id for channel in channels))
        raise FaqSeedingError(
            f"{len(channels)} {CHANNEL_TYPE} channels exist ({ids}). Name one explicitly."
        )
